...
from source.linear_segmentation_head import LinearClassifier
...
class HIPT(nn.Module):
    def __init__(
        ...
        dropout: float = 0.25,
        slide_pos_embed: Optional[DictConfig] = True,
        ...
        # self.classifier = nn.Linear(192, num_classes)
        self.classifier = LinearClassifier(192,num_labels=num_classes)
    ...
    def forward(self, x, pct: Optional[torch.Tensor] = None, pct_thresh: float = 0.0):
        ...
        # in nn.TransformerEncoderLayer, batch_first defaults to False
        # hence, input is expected to be of shape (seq_length, batch, emb_size)
        # x = self.global_transformer(x).squeeze(1)
        # att, x = self.global_attn_pool(x)
        # att = torch.transpose(att, 1, 0)
        # att = F.softmax(att, dim=1)
        # x_att = torch.mm(att, x)
        ...
        # logits = torch.nn.functional.interpolate(logits, size=(4096,4096), mode="bilinear", align_corners=False)
        return x
