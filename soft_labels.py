# -*- coding: utf-8 -*-
"""
Created on Wed Oct  2 10:14:19 2024
...
from SC_maps_utils import get_patch_centroids,calculate_distances_core_noncore,get_patches,\
                        calculate_nearest_neighbors_non_core,recontruct_image_from_patches,\
                            calculate_distances,calculate_iou,soft_smooth_expert_map,get_min_samples
...
sc_map1 = soft_smooth_expert_map(patch0_exp1)
sc_map2 = soft_smooth_expert_map(patch0_exp2)
sc_map3 = soft_smooth_expert_map(patch0_exp3)
sc_or_map = soft_smooth_expert_map(train_data[1][idx])

sc_map = np.zeros((6,4096,4096))
for i in range(6):
    sc_cl = sc_map1[i] + sc_map2[i] + sc_map3[i] #+ sc_or_map[i]
    sc_map[i] = (1/3)*sc_cl
    # np.save(f'D:/datasets/PANDA/expert_masks/soft_GT/{im}.npy',sc_map)
...
max_prob_sc_maps = np.max(sc_or_map,axis=0)
plt.imshow(max_prob_sc_maps,vmin=0.4,vmax=1,cmap=custom_cmap)
plt.colorbar() 
plt.show()

max_prob_sc_maps = sc_or_map[4]
...
prob = 0.4
while prob<0.7:
    sc_threshold = np.max(sc_map,axis=0).copy()
    sc_threshold[np.max(sc_map,axis=0)>prob] = 0
    # sc_threshold[np.max(sc_map,axis=0)<0.5] = 1
    max_prob_sc_maps = sc_threshold
    plt.imshow(max_prob_sc_maps)
    plt.colorbar()
    plt.title(f'prob: {float("{:.2f}".format(prob))}')
    plt.show()
    prob+=0.01



diff = train_data[1][idx] -np.argmax(sc_map,axis=0) 
diff[diff!=0] = 1
plt.imshow(diff)
plt.colorbar() 
plt.show()

diff1 = train_data[1][idx] -np.argmax(sc_map1,axis=0) 
diff1[diff1!=0] = 1
diff2 = train_data[1][idx] -np.argmax(sc_map2,axis=0) 
diff2[diff2!=0] = 1
diff3 = train_data[1][idx] -np.argmax(sc_map3,axis=0) 
diff3[diff3!=0] = 1

colors1 = ["indigo","green","orange",  "red"]  # Define custom colors
custom_cmap1 = LinearSegmentedColormap.from_list("custom_cmap", colors1)
diff_total = diff1+diff2+diff3
plt.imshow(diff_total,cmap=custom_cmap1)
plt.colorbar() 
plt.show()
...
expert_ann = np.argmax(sc_map,axis=0) 
cl,count = np.unique(expert_ann,return_counts=True)
cl_counts = dict(zip(cl,count))
...
diff3[diff3!=0] = 1
diff_total = diff1+diff2+diff3
diff_total[diff_total>0] = 1
prob = 0.4
ious_soft = {}
while prob <=0.7:
    sc_threshold = np.max(sc_map,axis=0).copy()
    sc_threshold[np.max(sc_map,axis=0)>prob] = 0
    sc_threshold[np.max(sc_map,axis=0)<prob] = 1
    
    iou = calculate_iou(diff_total,sc_threshold.astype('int64'))
    prob+=0.01
    ious_soft[prob] = iou
    
# plt.plot(list(ious.keys()),list(ious.values()))
