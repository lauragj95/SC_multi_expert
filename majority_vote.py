# -*- coding: utf-8 -*-
"""
Created on Wed Oct  2 12:19:57 2024
...
######## DBSCAN - ORIGINAL #########
dbscan1 = DBSCAN(eps=np.percentile(dist1,10), min_samples=1391).fit(list(cl_1.values()))
centroids_1 = dbscan1.components_
labels_1 = dbscan1.labels_

dbscan2 = DBSCAN(eps=np.percentile(dist2,10), min_samples=97).fit(list(cl_2.values()))
centroids_2 = dbscan2.components_
labels_2 = dbscan2.labels_

dbscan3 = DBSCAN(eps=np.percentile(dist3,10), min_samples=34).fit(list(cl_3.values()))
centroids_3 = dbscan3.components_
labels_3 = dbscan3.labels_

dbscan4 = DBSCAN(eps=np.percentile(dist4,10), min_samples=185).fit(list(cl_4.values()))
centroids_4 = dbscan4.components_
labels_4 = dbscan4.labels_

dbscan5 = DBSCAN(eps=np.percentile(dist5,10), min_samples=5).fit(list(cl_5.values()))
centroids_5 = dbscan5.components_
labels_5 = dbscan5.labels_



###### GET CLUSTER PATCHES AND NN TO NON-CORE POINTS #####

core2,non_core2 = get_patch_centroids(cl_2,centroids_2,labels_2)
...
core3,non_core3 = get_patch_centroids(cl_3,centroids_3,labels_3)
...
core4,non_core4 = get_patch_centroids(cl_4,centroids_4,labels_4)
...
outliers2 = get_outliers(labels_2,cl_2)
outliers3 = get_outliers(labels_3,cl_3)
outliers4 = get_outliers(labels_4,cl_4)

##### INTRODUCE NOISE ######
## Mix 2-3 ##

random.seed(0)
cl2_aux_rand = copy.deepcopy(list(cl_2_aux.items()))
cl3_aux_rand = copy.deepcopy(list(cl_3_aux.items()))
...
random.seed(10)
cl3_aux_rand = copy.deepcopy(list(cl_3_aux.items()))
cl4_aux_rand = copy.deepcopy(list(cl_4_aux.items()))
random.shuffle(cl4_aux_rand)
random.shuffle(cl3_aux_rand)
change4_3 = dict(cl4_aux_rand[:int(0.3*len(cl_4))])
change3_4 = dict(cl3_aux_rand[:int(0.3*len(cl_3))])
...
patch0_exp1 = imgs_exp1[1]
patch0_exp2 = imgs_exp2[1]
patch0_exp3 = imgs_exp3[1]

plt.imshow(imgs_exp1[1])
plt.colorbar() 
plt.show()


def sc_expert_map(expert_mask):
...
sc_map1 = sc_expert_map(patch0_exp1)
sc_map2 = sc_expert_map(patch0_exp2)
sc_map3 = sc_expert_map(patch0_exp3)
sc_or_map = sc_expert_map(train_data[1][1])


sc_map = np.zeros((6,4096,4096))
for i in range(6):
    sc_cl = sc_map1[i] + sc_map2[i] + sc_map3[i]
    sc_map[i] = sc_cl
    
soft = torch.nn.Softmax()
soft_sc_map = soft(torch.from_numpy(sc_map)).cpu().detach().numpy()
...
max_prob_sc_maps = np.max(soft_sc_map,axis=0)
plt.imshow(max_prob_sc_maps)
plt.colorbar() 
plt.show()
...
for i in range(len(train_data[1])):
    if 5 in np.unique(train_data[1][i]):
        print(i)
    

diff = train_data[1][2] -np.argmax(sc_map,axis=0) 
diff[diff!=0] = 1
plt.imshow(diff)
plt.colorbar() 
plt.show()
...
# for patch in list(change2_3.keys())[:10]:
plt.figure(figsize=(10, 10))
# norm=plt.Normalize(1,6)
# idx = patch.split('_')

expert_ann = train_data[1][1]
cl,count = np.unique(expert_ann,return_counts=True)
cl_counts = dict(zip(cl,count))
map_name = matplotlib.colors.LinearSegmentedColormap.from_list("", ["black","red","blue", "green","yellow","pink"])
...
output = loss(soft(torch.from_numpy(sc_or_map.astype(float))),soft(torch.from_numpy(sc_map)))
