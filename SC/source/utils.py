...
def make_weights_for_balanced_classes(images, nclasses,argmax=False):
    ...
    weight_per_class[0] = 1.0                          
    weight = [0] * len(images)                                              
    for idx,img in enumerate(images):    
        labels = np.unique(img)                  
        for i,label in enumerate(labels):                                          
            weight[idx] += weight_per_class[int(label)]
...
#         labels,counts = np.unique(img,return_counts=True)                  
#         for i,label in enumerate(labels):                                          
#             weight[idx] += weight_per_class[int(label)]*counts[i] 
#     return torch.DoubleTensor(weight)
...
def train_on_regions(
    ...
    #     writer.add_scalar(f'f1_score_{str(i)}', f1, epoch)
    train_loss = epoch_loss / len(train_loader)
    # results["loss"] = train_loss
    writer.add_scalar('train_loss', train_loss, epoch)
    # writer.add_image('true',labels[0].cpu().detach().numpy())
    # writer.add_image('pred',preds[0].cpu().detach().numpy())
...
def tune_on_regions(
    ...
    tune_loss = epoch_loss / len(loader)
    # results["loss"] = tune_loss
    writer.add_scalar('valid_loss', tune_loss, epoch)
    # writer.add_scalar('valid_auc', results["auc"], epoch+1)
    writer.flush()
